...
const ChristmasHistoryModal = ({
    ...
    // Settings Tab State
    const [activeTab, setActiveTab] = useState('history'); // 'history' | 'menu'
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            >
                {/* Header & Tabs */}
                <div className="bg-[#0f172a] text-white flex flex-col shadow-md shrink-0">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${activeTab === 'history' ? 'bg-[#D97706] text-white' : 'hover:bg-white/10 text-white/60'}`}
                                >
                                    Order History
                                </button>
                                <button
                                    onClick={() => setActiveTab('menu')}
                                    className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${activeTab === 'menu' ? 'bg-[#D97706] text-white' : 'hover:bg-white/10 text-white/60'}`}
                                >
                                    Menu & Prices
                                </button>
                            </div>
                    ...
                    <div className="flex-1 overflow-auto p-6 bg-gray-50">
                        <div className="max-w-4xl mx-auto">
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    <h3 className="text-lg font-bold text-gray-900">Menu & Pricing</h3>
                                    <p className="text-sm text-gray-500">Manage items and location-based prices.</p>
                                </div>
                                <button
                                    onClick={() => {
                                        setEditingItem(null);
                                    ...
                                    className="px-4 py-2 bg-[#0f172a] text-white rounded-lg font-bold text-sm shadow hover:bg-[#1e293b]"
                                >
                                    + Add Item
                                </button>
                            </div>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-gray-50 border-b border-gray-200">
                                        <tr>
                                            <th className="px-6 py-3 text-xs font-bold text-gray-500 uppercase">Item Info</th>
                                            <th className="px-6 py-3 text-xs font-bold text-gray-500 uppercase text-right">Legazpi Price</th>
                                            <th className="px-6 py-3 text-xs font-bold text-gray-500 uppercase text-right">Sorsogon Price</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {menuConfig.map((item, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50 group">
                                                ...
                                                <td className="px-6 py-4 text-right font-medium text-gray-700">₱{item.priceLeg}</td>
                                                <td className="px-6 py-4 text-right font-medium text-gray-700">₱{item.priceSor}</td>
                                                <td className="px-6 py-4 text-right">
                                                    <button
                                                        onClick={() => {
                                                            setEditingItem(idx); // Use index for simplicity since SKU can change
                                                            setTempItem({ ...item });
                                                            setIsItemModalOpen(true);
                                                        }}
