...
const DaetStorage = () => {
    ...
    // Filter transfers for Daet Storage
    const daetTransfers = transferOrders
        .filter(order => order.to_location === 'Daet Storage' || order.destination === 'Daet Storage')
    ...
    const handleStatusChange = async (id, newStatus) => {
        ...
        const order = daetTransfers.find(o => o.id === id);
    ...
    const filteredAndSortedInventory = daetInventory
        .filter(item => {
            const matchesSearch =
    ...
    return (
        <div className="fade-in">
            ...
            <div style={{
                ...
                <div>
                    <h2 className="page-title" style={{ marginBottom: '0.25rem' }}>Daet Storage</h2>
                ...
                <div className="flex gap-2">
                    ...
                    >
                        ...
                        Daet Storage History
                    </button>
                    <button
                        onClick={() => window.open('/print-stocks/Daet Storage', '_blank')}
                        className="submit-btn"
            ...
            {showTransferHistory && (
                <div className="modal-overlay" onClick={() => setShowTransferHistory(false)}>
                    <div className="modal-content large-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '1200px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                        <div className="modal-header">
                            <div>
                                <h2 className="modal-title">Daet Storage History</h2>
                                <p className="page-subtitle" style={{ margin: 0 }}>Transaction History</p>
                            </div>
                            <button className="close-btn" onClick={() => setShowTransferHistory(false)}>
                                <X size={24} />
                            </button>
                        </div>
                        <div className="modal-body" style={{ flex: 1, overflow: 'auto', padding: '0' }}>
                            <div className="table-container">
                                <table className="inventory-table">
                                    ...
                                    <tbody>
                                        {daetTransfers.length === 0 ? (
                                            <tr>
                                                <td colSpan={8} className="empty-state" style={{ padding: '2rem', textAlign: 'center' }}>
                                                    No transfers found for Daet Storage.
                                                </td>
                                            </tr>
                                        ) : (
                                            daetTransfers.map(order => (
                                                <tr key={order.id}>
                                                    ...
                                                    <td className="font-medium">{order.destination || order.to_location}</td>
                        ...
                        <div className="modal-body">
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium">{selectedOrder.destination || selectedOrder.to_location}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium">{new Date(selectedOrder.date || selectedOrder.created_at).toLocaleString()}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className={`status-badge status-${selectedOrder.status?.toLowerCase().replace(' ', '-')}`}>{selectedOrder.status}</p>
                                </div>
                            </div>
