...
const LegazpiStorage = () => {
    ...
    const legazpiTransfers = transferOrders
        .filter(order => order.to_location === 'Legazpi Storage' || order.destination === 'Legazpi Storage')
    ...
    const handleStatusChange = async (id, newStatus) => {
        ...
        const order = legazpiTransfers.find(o => o.id === id);
            ...
            {showTransferHistory && (
                <div className="modal-overlay" onClick={() => setShowTransferHistory(false)}>
                    <div className="modal-content large-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '1200px', width: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                        <div className="modal-header">
                            <div>
                                <h2 className="modal-title">Legazpi Storage History</h2>
                        ...
                        <div className="modal-body" style={{ flex: 1, overflow: 'auto', padding: '0' }}>
                            <div className="table-container">
                                <table className="inventory-table">
                                    ...
                                    <tbody>
                                        {legazpiTransfers.length === 0 ? (
                                            <tr>
                                                <td colSpan={8} className="empty-state" style={{ padding: '2rem', textAlign: 'center' }}>
                                                    No transfers found for Legazpi Storage.
                                                </td>
                                            </tr>
                                        ) : (
                                            legazpiTransfers.map(order => (
                                                <tr key={order.id}>
